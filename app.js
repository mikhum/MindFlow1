...
function checkPotentialParent(draggedId, absX, absY) {
    let closestId = null;
    let minDistance = 180; // Detection threshold in pixels (increased from 140 for easier snapping)
    ...
    Object.keys(nodes).forEach(id => {
        ...
        const distance = Math.hypot(absX - candidateAbs.x, absY - candidateAbs.y);
    ...
    if (hoveredParentId !== closestId) {
        ...
        hoveredParentId = closestId;
        
        // Highlight new potential parent
        if (hoveredParentId) {
...
function handleKeyDown(e) {
    // If user is typing in a text field or modal is active, skip global shortcuts
    if (editingNodeId || helpModal.classList.contains("active")) {
        return;
    }
...
// Save current map to LocalStorage under a named key
function handleSaveMap() {
    let name = currentMapName;
    if (!name) {
        const rootNodeText = nodes.root.text.trim();
        name = prompt("Enter a name for this mindmap:", rootNodeText || "My Mindmap");
        if (name === null) return; // User cancelled
        name = name.trim() || "My Mindmap";
    }
    ...
    alert(`Map "${name}" successfully saved!`);
    loadMapList();
}
    ...
    reader.onload = function(evt) {
        try {
            ...
            if (data && data.nodes && data.nodes.root) {
                nodes = data.nodes;
                relationships = data.relationships || [];
                currentMapName = data.name || file.name.replace(/\.[^/.]+$/, "");
                ...
                alert("Mindmap successfully imported!");
            } else {
                alert("Invalid mindmap file format: root node is missing.");
            }
        } catch (err) {
            alert("Error parsing mindmap file: " + err.message);
        }
    };
